...
fn test_if_expression() {
    ...
    match &program.statements[0] {
        Statement::ExpressionStatement { expression, .. } => {
            match expression {
                Expression::IfExpression { condition, consequence, alternative, .. } => {
                    test_condition((**condition).clone(), "x < 5");
                    match consequence {
                        Expression::BlockStatement { statements, .. } => {
                            if statements.len() != 1 {
                                panic!("More than 1 statement found: {}", statements.len());
                            }
                        }
                    }
                }
        ...
        stmt => panic!("This statement is not ExpressionStatement: got {:?}", stmt),
    }
}
...
fn test_condition(expression: Expression, cond: &str) {
    if expression.string().as_str() != cond {
        panic!("Condition is different: expect {}, got {}", expression.string(), cond);
    }
}
